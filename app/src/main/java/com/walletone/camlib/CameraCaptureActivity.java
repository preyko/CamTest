...
public class CameraCaptureActivity extends Activity
    ...
    private static final String TAG = CameraCaptureActivity.class.getName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int currentGlesVersion = GlUtil.getGLESVersion(this);
        if (currentGlesVersion < 0x20000) { // uuups, GLES isn't supported
            new AlertDialog.Builder(this).
                    setTitle("Error").
                    setMessage("We are sorry, but your device doesn't support OpenGL >= 2.").
                    ...
                    show();
        } else if (currentGlesVersion > 0x30000) {
            currentGlesVersion = 3;
        } else {
            currentGlesVersion = 2;
        }


        // Initialize views fields
        mFPSView = (TextView) findViewById(R.id.fps_view);
        mPTView = (TextView) findViewById(R.id.processing_time_view);
        ...
        */
        ...
        final CheckBox checkBox = (CheckBox)findViewById(R.id.save_frame_cb);
        checkBox.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                mGLView.queueEvent(new Runnable() {
                    @Override
                    public void run() {
                        mRenderer.changeFrameSaving(checkBox.isChecked());
                    }
                });
            }
        });
        ...
        mGLView.setEGLContextClientVersion(currentGlesVersion);
    ...
    protected void onResume() {
        ...
        super.onResume();
        // updateControls();
        openCamera(1280, 720, 20000);      // updates mCameraPreviewWidth/Height
        ...
        switch (parent.getId()) {
            case R.id.convert_method_spinner: {
                ...
                mGLView.queueEvent(new Runnable() {
                    @Override
                    public void run() {
                        mRenderer.changeTextureConvertMethod(method);
                    }
                });
            } break;
            case R.id.convert_mode_spinner: {
                ...
                mGLView.queueEvent(new Runnable() {
                    @Override
                    public void run() {
                        mRenderer.changeTextureConvertMode(mode);
                    }
                });
            } break;
        }
     ...
     */
    private void openCamera(int desiredWidth, int desiredHeight, int minimumFps) {
        if (mCamera != null) {
        ...
        CameraUtils.chooseCameraDisplayOrientation(this, 0, mCamera);

        // Set the setRecordingHint before to choose new fps
        {
        ...
        // Set desired preview size and fps range
    ...
    /**
     * Notify activity about current FPS
     * @param fps - current fps
     */
    private void handleCurrentFPS(float fps) {
        mFPSView.setText("" + fps);
    }

    /**
     * Notify activity about current Processing Time
     * @param pt - current processing time
     */
    private void handleCurrentPT(long pt) {
        mPTView.setText("" + pt);
    }
    ...
    static class CameraHandler extends Handler {
        public static class RatesHolder {
            public RatesHolder(long pt, float f) {
                processing_time = pt;
                fps = f;
            }
        ...
        public static final int MSG_SET_CURRENT_RATES = 1;
        public static final int MSG_SET_CURRENT_MODE = 2;
        public static final int MSG_SET_CURRENT_METHOD = 3;
        ...
        public void handleMessage(Message inputMessage) {
            ...
            switch (what) {
                case MSG_SET_SURFACE_TEXTURE:
                    ...
                    break;
                case MSG_SET_CURRENT_RATES:
                    RatesHolder holder = (RatesHolder) inputMessage.obj;
                    activity.handleCurrentFPS(holder.fps);
...
class CameraSurfaceRenderer implements GLSurfaceView.Renderer {
    ...
    // FPS and PT fields
    private static final int REPORT_RATE = 25;
     ...
     * @param checked
     */
    public void changeFrameSaving(boolean checked) {
        mSaveFrame = checked;
    }
    ...
    long mPrevTimestamp = 0;

    @Override
    public void onDrawFrame(GL10 unused) {
        if (VERBOSE) Log.d(TAG, "onDrawFrame tex=" + mTextureId + ", prev call time diff: " +
                (System.currentTimeMillis() - mPrevTimestamp));
        mPrevTimestamp = System.currentTimeMillis();
        ...
        if (mConvertModeChanged || mConvertMethodChanged) {
            mFPSCounter.reset();
            mLastProcessingTime = 0;
        }
        if (mConvertMethodChanged) {
            mConvertMethodChanged = false;
            CamTestLib.setTextureConvertMethod(mCurrentConvertMethod);

            mCameraHandler.sendMessage(mCameraHandler.obtainMessage(
                    CameraCaptureActivity.CameraHandler.MSG_SET_CURRENT_METHOD,
                    mCurrentConvertMethod.ordinal(), 0));
        }
        if (mConvertModeChanged) {
            mConvertModeChanged = false;
            CamTestLib.setTextureConvertMode(mCurrentConvertMode);
            mCameraHandler.sendMessage(mCameraHandler.obtainMessage(
                    CameraCaptureActivity.CameraHandler.MSG_SET_CURRENT_MODE,
                    mCurrentConvertMode.ordinal(), 0));
        }

        // Convert texture
        CamTestLib.nativeSaveTexture(new CamTestLib.GlTexture(mTextureId,
                        mFullScreen.
                                getProgram().
                                getTextureTarget(),
                        mSTMatrix),
                mIncomingWidth, mIncomingHeight, mSaveFrame);
        ...
        {
            long pt = System.currentTimeMillis() - mPrevTimestamp;

            if (mLastProcessingTime != 0)
                mLastProcessingTime = (mLastProcessingTime + pt) / 2;
            else
                mLastProcessingTime = pt;

            if (mFrameCount % REPORT_RATE == 0) {
                Log.d(TAG, "Reporting rates: PT: " + mLastProcessingTime + ", FPS: "
                                                   + mFPSCounter.getFPS());
                Message message = mCameraHandler.
                        obtainMessage(CameraCaptureActivity.CameraHandler.MSG_SET_CURRENT_RATES,
                                      new CameraCaptureActivity.CameraHandler.RatesHolder
                                                                   (mLastProcessingTime,
                                                                    mFPSCounter.getFPS()));
        ...
        if (mFrameCount++ > Integer.MAX_VALUE - 1000)
            mFrameCount = 0;
    }
